...
- Webview CSP is `default-src 'self'`; `connect-src` adds only the IPC origins (`ipc:`, and `http://ipc.localhost` on Windows), which carry commands, not network traffic. Tauri serves that header from the asset protocol, so `tauri dev` on the Vite URL enforces no CSP - verify against a build.
